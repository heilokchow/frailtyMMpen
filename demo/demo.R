...
m = 100
all_rs = matrix(0, ncol = 12, nrow = m)

for (i in 1:m) {
  yy = sample(coef = c(rep(-2,6), rep(-1,6), rep(1,6), rep(2,6), rep(3,6)), init.var = 0.25, a = 50, b = 10, frailty = "LogN")
  y = yy$y 
  d = yy$d
  X = yy$X
  
  rs1 = frailtyMM(y, X, d)
  all_rs[i, ] = c(rs1$coef, rs1$est.tht, rs1$likelihood)
}
...
ell[k]= logLikihood(y, X, d, coef, lambda, est.tht, frailty = "InvGauss")
error = 3

start = proc.time()[1]
while(error > 0.000001) {
  
  rs = EMprocess(y, X, d, coef, lambda, est.tht, frailty = "InvGauss") 
  coef = rs$coef
  lambda = rs$lambda
  est.tht = rs$est.tht
  
  ell[k+1] = logLikihood(y, X, d, coef, lambda, est.tht, frailty = "InvGauss")
