frailtyMM_CL <- function(y, X, d, coef.ini = NULL, est.tht.ini = NULL, lambda.ini = NULL, frailty = "LogN", power = NULL, penalty = NULL, tune = NULL) {
  ...
  while(error > 0.000001 && num < 1000) {
    ...
    rs2 = MMprocess_CL(y, X, d, coef1, lambda, est.tht1, frailty = frailty, power = power, penalty = penalty, tune = tune)
    coef2 = rs2$coef
    ...
    rs = MMprocess_CL(y, X, d, coef, lambda, est.tht, frailty = frailty, power = power, penalty = penalty, tune = tune) 
    coef = rs$coef
    est.tht = rs$est.tht
    lambda = rs$lambda
...
frailtyMM_ME <- function(y, X, d, coef.ini = NULL, est.tht.ini = NULL, lambda1.ini = NULL, lambda2.ini = NULL, frailty = "LogN", penalty = NULL, tune = NULL) {
  ...
  while(error > 0.000001) {
    ...
    rs2 = MMprocess_ME(y, X, d, coef1, lambda1, lambda2, est.tht1, frailty = frailty, penalty = penalty, tune = tune)
    coef2 = rs2$coef
    est.tht2 = rs2$est.tht
    lambda1 = rs2$lambda1
    lambda2 = rs2$lambda2
    ...
    coef = coef - (2*al_be*u_be - al_be^2*v_be)
    est.tht = est.tht2
    rs = MMprocess_ME(y, X, d, coef, lambda1, lambda2, est.tht, frailty = frailty, penalty = penalty, tune = tune) 
    coef = rs$coef
    est.tht = rs$est.tht
    lambda1 = rs$lambda1
    lambda2 = rs$lambda2
